...
namespace PygmyMonkey.ColorPalette
{
	public class ColorPaletteImportRenderer
	{
		...
		private void importPaletteFromFile(string filePath)
		{
			...
			default:
				throw new UnityException("Sorry we do not support this file type. Please contact us at tools@pygmymonkey.com and we'll add support for whatever you want :D");
			}
		...
		private void importPaletteFromURL(string url)
		{
			...
			try
			{
				...
				{
					throw new UnityException("Sorry we do not support downloading palettes from the website " + uri.Host + " for now. Please contact us at tools@pygmymonkey.com and we'll add support for whatever you want :D");
				}
			}
			catch (Exception e)
			{
